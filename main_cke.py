...
def train(args):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        for iter in range(1, n_batch + 1):
            ...
            batch_loss = model('train', cf_batch_user, cf_batch_pos_item, cf_batch_neg_item, kg_batch_head, kg_batch_relation, kg_batch_pos_tail, kg_batch_neg_tail).mean()
            batch_loss.backward()
            optimizer.step()
            optimizer.zero_grad()
        ...
        if (epoch % args.evaluate_every) == 0:
            time1 = time()
            _, precision, recall, ndcg, HR = evaluate(model, data.train_user_dict, data.test_user_dict, user_ids_batches, item_ids, args.K)
